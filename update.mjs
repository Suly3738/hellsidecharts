// HellsideCharts – notowanie utworów z kanału YouTube Hellside Records.
...
async function fetchViaApi(apiKey) {
  ...
  const chRes = await get('channels', handle
    ...
    : { part: 'snippet,contentDetails,statistics', id: cfg.channelId });
  ...
  const channelStats = {
    ...
    channelViews: Number(ch.statistics?.viewCount ?? 0),
    createdAt: ch.snippet.publishedAt ?? null,
  };
  return { channelTitle: ch.snippet.title, raw, channelStats };
}
...
async function fetchViaInnertube() {
  ...
  return { channelTitle, raw, channelStats: {} };
}
...
// beaty w tle po intrze: pula id z config.introTracks (strona losuje jeden przy każdym wejściu); celowo bez tytułów/autorów – nigdzie nie pokazujemy, co gra
const introTracks = [].concat(cfg.introTracks || cfg.introTrack || []).filter(Boolean);
...
// ---------- 4b. dodatkowe zestawienia ----------

const fmtDate = (iso) => iso ? new Date(iso + 'T00:00:00').toLocaleDateString('pl-PL', { day: 'numeric', month: 'short', year: 'numeric' }) : '';

// największe przyrosty wyświetleń tydzień do tygodnia (tylko gdy jest punkt odniesienia)
const rising = prev
  ? allTimePool
      .map(v => ({ v, delta: typeof prev.views?.[v.id] === 'number' ? v.views - prev.views[v.id] : null }))
      .filter(x => x.delta !== null && x.delta > 0)
      .sort((a, b) => b.delta - a.delta || byViews(a.v, b.v))
      .slice(0, cfg.risingSize ?? 10)
      .map(({ v, delta }, i) => ({ pos: i + 1, id: v.id, artist: v.artist, song: v.song, title: v.title, views: v.views, viewsDelta: delta, url: v.url, thumb: v.thumb, publishedText: v.publishedText,
        pct: prev.views[v.id] > 0 ? Math.round(delta / prev.views[v.id] * 1000) / 10 : null }))
  : [];

// ranking wykonawców: suma wyświetleń, liczba utworów, najlepszy utwór, najwyższe miejsce All Time
const labelViews = allTimePool.reduce((s, v) => s + v.views, 0);
const artistMap = new Map();
for (const v of allTimePool) {
  const names = (v.artist || 'Nieznany').split(/s*(?:,|&|x|ft.?|feat.?)s*/i).map(n => n.trim()).filter(Boolean);
  for (const name of names) {
    const key = name.normalize('NFKC').toLowerCase();
    const a = artistMap.get(key) ?? { name, tracks: [], views: 0 };
    a.tracks.push(v); a.views += v.views;
    artistMap.set(key, a);
  }
}
const allTimePos = Object.fromEntries(allTime.map((v, i) => [v.id, i + 1]));
const artists = [...artistMap.values()]
  .sort((a, b) => b.views - a.views || b.tracks.length - a.tracks.length)
  .map((a, i) => {
    const best = [...a.tracks].sort(byViews)[0];
    const newest = [...a.tracks].sort((x, y) => x.order - y.order)[0];
    const prevViews = prev ? a.tracks.reduce((s, v) => s + (typeof prev.views?.[v.id] === 'number' ? prev.views[v.id] : v.views), 0) : null;
    const bestPos = Math.min(...a.tracks.map(v => allTimePos[v.id] ?? Infinity));
    return {
      pos: i + 1, name: a.name, tracks: a.tracks.length, views: a.views,
      share: labelViews ? Math.round(a.views / labelViews * 1000) / 10 : 0,
      viewsDelta: prevViews !== null ? a.views - prevViews : null,
      bestPos: Number.isFinite(bestPos) ? bestPos : null,
      top: { id: best.id, song: best.song, title: best.title, views: best.views, url: best.url, thumb: best.thumb },
      newest: { id: newest.id, song: newest.song, publishedText: newest.publishedText },
    };
  });

// historia miejsca #1 w Top 10 Now: kolejne „panowania” (od kiedy, ile dni)
const titleById = Object.fromEntries(videos.map(v => [v.id, { artist: v.artist, song: v.song, title: v.title, thumb: v.thumb, url: v.url }]));
const reigns = [];
for (const s of history.snapshots) {
  const id = s.now?.[0]; if (!id) continue;
  const last = reigns[reigns.length - 1];
  if (last && last.id === id) { last.to = s.date; last.days = Math.round((new Date(s.date) - new Date(last.from)) / 864e5) + 1; }
  else reigns.push({ id, from: s.date, to: s.date, days: 1, ...(titleById[id] ?? { title: id }) });
}
reigns.reverse();
...
const totals = {
  labelViews,
  tracks: allTimePool.length,
  artists: artists.length,
  subscribers: channelStats?.subscribers ?? null,
  channelViews: channelStats?.channelViews ?? null,
  channelCreatedAt: channelStats?.createdAt ?? null,
  viewsDelta: prevTotal !== null ? labelViews - prevTotal : null,
  firstRelease: allTimePool.length ? [...allTimePool].sort((a, b) => b.order - a.order)[0].publishedText : '',
  snapshots: history.snapshots.length,
  firstSnapshot: history.snapshots[0]?.date ?? chartDate,
};
...
const data = {
  ...
  rising,
  artists,
  reigns: reigns.map(r => ({ ...r, fromText: fmtDate(r.from), toText: fmtDate(r.to) })),
  latest: latest.map(v => ({
